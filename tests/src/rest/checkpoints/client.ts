...
import { createCheckpointParams, createScheduleParams, modifyDistributionCheckpointParams } from "./params";
  ...
  export class Checkpoints {
    ...
    public async createCheckpoint(asset: string, params: ReturnType<typeof createCheckpointParams>): Promise<PostResult> {
    ...
    public async createSchedule(asset: string, params: ReturnType<typeof createScheduleParams>): Promise<PostResult> {
      return this.client.post(`/assets/${asset}/checkpoints/schedules/create`, params);
    }

    // GET /assets/{asset}/checkpoints/{id}/balances
    public async getCheckpointBalances(asset: string, id: string): Promise<ResultSet<{ identity: string, balance: string }>> {
      return this.client.get(`/assets/${asset}/checkpoints/${id}/balances`);
    }

    // GET /assets/{asset}/checkpoints/{id}/balances/{did}
    public async getCheckpointBalanceForDid(asset: string, id: string, did: string): Promise<unknown> {
    ...
    public async deleteSchedule(asset: string, id: string, params: TxBase): Promise<PostResult> {
      return this.client.post(`/assets/${asset}/checkpoints/schedules/${id}/delete?signer=${params.options.signer}&processMode=${params.options.processMode}`, {});
    }
    ...
    public async modifyDistributionCheckpoint(asset: string, id: string, params: ReturnType<typeof modifyDistributionCheckpointParams>): Promise<PostResult> {
      return this.client.post(`/assets/${asset}/corporate-actions/dividend-distributions/${id}/modify-checkpoint`, params);
    }
  }
