...
describe("Checkpoints Controller", () => {
    ...
    describe("Checkpoint Management", () => {
        it("complete the pre-requisites", async () => {
            ...
            expect((transferToHolder1Tx as any).instruction).toBeDefined();


            // Use the correct property for the instruction ID (assuming it's 'id' from the API response)
            await restClient.settlements.affirmInstruction((transferToHolder1Tx as any).instruction, { options: { processMode: ProcessMode.Submit, signer: holder1.signer } });
            ...
            // should have created an instruction
            expect((transferToHolder2Tx as any).instruction).toBeDefined();

            const txData2 = await restClient.settlements.affirmInstruction((transferToHolder2Tx as any).instruction, { options: { processMode: ProcessMode.Submit, signer: holder2.signer } }) as RestSuccessResult
        ...
        it("should create a checkpoint", async () => {
            ...
            expect((createCheckpointTx.checkpoint as any).id).toBeDefined();

            checkpointId = (createCheckpointTx.checkpoint as any).id as string;
        });
        ...
        it("should get checkpoint balances", async () => {
            const { results: checkpointBalances } = await restClient.checkpoints.getCheckpointBalances(assetId, checkpointId);
    ...
    describe("Checkpoint Schedule Management", () => {
        it("should create a checkpoint schedule", async () => {
            ...
            expect(createScheduleTx.schedule).toMatchObject(expect.objectContaining({
                id: expect.any(String),
                asset: assetId,
                expiryDate: expect.any(String),
                pendingPoints: expect.arrayContaining([
                    expect.any(String),
                ]),
            ...
            scheduleId = (createScheduleTx.schedule as any).id as string;
        });
        ...
        it("should fetch schedule by ID and get details", async () => {
            ...
            expect((schedule as any)).toHaveProperty('id', scheduleId);
            expect((schedule as any)).toHaveProperty('asset', assetId);
            expect((schedule as any)).toHaveProperty('expiryDate');
            expect((schedule as any)).toHaveProperty('nextCheckpointDate');
            expect((schedule as any)).toHaveProperty('pendingPoints');
            expect(Array.isArray((schedule as any).pendingPoints)).toBe(true);
            expect((schedule as any).pendingPoints.length).toBeGreaterThan(0);
            expect((schedule as any)).toHaveProperty('remainingCheckpoints');
            expect(typeof (schedule as any).remainingCheckpoints).toBe('string');
        });
        ...
        it("should get checkpoints originated by a schedule", async () => {
            ...
            const scheduleCheckpoints = await restClient.checkpoints.getScheduleCheckpoints(assetId, scheduleId);
        ...
        it("should get checkpoint complexity for the asset", async () => {
            ...
            expect(complexity).toMatchObject(expect.arrayContaining([
                expect.objectContaining({
        ...
        it("should get schedule complexity for the asset", async () => {
            ...
            expect(complexity).toBeDefined();
            expect(complexity).toMatchObject(expect.objectContaining({
                complexity: "3"
            }));
        });
