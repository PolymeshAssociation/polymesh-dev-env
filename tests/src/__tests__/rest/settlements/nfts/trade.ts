...
describe("Create and trading an NFT", () => {
  ...
  beforeAll(async () => {
    ...
    const params = issueNftParams(
      assetId,
      [{ type: "Local", id: "1", value: "test value" }],
      {
        options: { processMode: ProcessMode.Submit, signer },
      }
    );
  ...
  it("should allow instruction with NFT leg to be viewed", async () => {
    const pendingResult = await restClient.identities.getPendingInstructions(
      collector.did
    );
  ...
  it("should allow affirmation to be withdrawn", async () => {
    const result = await restClient.settlements.withdrawAffirmation(
      instructionId,
      {
  ...
  it("should allow instruction to be affirmed by collector", async () => {
    const result = await restClient.settlements.affirmInstruction(
      instructionId,
      {
  ...
  it("should allow instruction to be rejected", async () => {
    const result = await restClient.settlements.rejectInstruction(
      instructionId,
      {
        options: { processMode: ProcessMode.Submit, signer: collector.signer },
      }
