...
describe("Ticker Reservations", () => {
  ...
  it("should reserve a ticker", async () => {
    ...
    expect(result).toEqual(
      assertTagPresent(expect, "asset.registerUniqueTicker")
    );
  });
  ...
  it("should list an Identities reservations before accepting", async () => {
    const reservations =
      await restClient.tickerReservations.getIdentityReservations(receiver.did);
  ...
  it("should accept the transfer", async () => {
    ...
    expect(result).toEqual(
      assertTagPresent(expect, "asset.acceptTickerTransfer")
    );
  });
  ...
  it("should not be able to reserve an already reserved ticker", async () => {
    ...
    const result = (await restClient.tickerReservations.reserve(
      params
    )) as RestErrorResult;
